...
.. warning:: 较小的缓冲区可以减少延迟，但会增加 CPU 使用率并增加音频中断的风险，且必须在 :ref:`init()<FmodSystem-init>` 之前调用！
...
.. note:: 如果 ``loop`` 为 ``true``，当录音达到缓冲区末尾时，会从音频缓冲区的开头重新开始录音
