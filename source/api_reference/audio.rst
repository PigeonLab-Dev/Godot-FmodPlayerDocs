...
.. list-table::
  ...
  * - `int`_
    - mode_flags
    - 1
    - 创建模式标志，用于确定音频的加载和播放方式。这是 :ref:`CreateMode<CreateMode>` 值的位掩码。当这些标志被更改时，内部的 :ref:`FmodSound<FmodSound>` 会被释放，并将在下一次调用 :ref:`get_sound()<FmodAudioStream-get_sound>` 时使用新的标志重新创建
...
清除音频数据并释放内部 :ref:`FmodSound<FmodSound>` 资源。这将释放内存并将流重置为空状态。流在再次播放之前需要重新加载
